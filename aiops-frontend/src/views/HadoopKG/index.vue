<template>
<div id="hadoopKG" style="height:100%;">


<div style="margin-top: 15px;width:80%;">
  <el-input id='txt' placeholder="请输入内容" v-model="cyphertext" class="input-with-select">
    <el-button slot="append" @click="cypherrun" icon="el-icon-search"></el-button>
  </el-input>
</div>


<div id="graphcontainer" class="graphcontainer" style="height:100%;"></div>
...
export default {
    ...
    data() {
        ...
        // uploadimageurl: "http://localhost:8080/" + "qiniu/upload",
        uploadimageparam: {},
        nodeimagelist: [],
        netimageurl: '',
        ...
        headers: {},
        uploadurl: "http://localhost:8080/" + "importgraph"
        }
    },
    filters: {
    ...
    mounted() {
        ...
        if(typeof(this.$route.query.name) == 'undefined'){
            ...
            $('#txt').val(this.$route.query.name);
            this.cypherrun();
        }
    },
    created() {
        this.graph.nodes = [{"name":"hadoop","uuid":"1044","label":"hadoop"},{"name":"tools","uuid":"26","label":"tools"},{"name":"configuration_file","uuid":"38","label":"configuration_file"},{"name":"component","uuid":"42","label":"component"},{"label":"command","name":"command","uuid":"1064"},{"label":"command","parameter":"| -block block-file : Absolute path for the block file on the local file system of the data node. | -out output-metadata-file : Absolute path for the output metadata file to store the checksum computation result from the block file. ","name":"computeMeta","detail":"Usage: hdfs debug computeMeta -block <block-file> -out <output-metadata-file>Compute HDFS metadata from block files. If a block file is specified, we will compute the checksums from the block file, and save it to the specified output metadata file.NOTE: Use at your own risk! If the block file is corrupt and you overwrite it’s meta file, it will show up as ‘good’ in HDFS, but you can’t read the data. Only use as a last measure, and when you are 100% certain the block file is good.","uuid":"140"}];
	    this.graph.links = [{"sourceid":"1044","targetid":"26","name":"include","uuid":"1211"},{"sourceid":"1044","targetid":"38","name":"include","uuid":"43"},{"sourceid":"1044","targetid":"42","name":"include","uuid":"47"},{"sourceid":"1044","targetid":"1064","name":"include","uuid":"1044"}];
    },
    methods: {
    	btntipsclose(){
    		this.tipsshow=false;
        ...
        cypherrun(){
            var _this = this;
            if($('#txt').val()==""){
                _this.$message.error("请输入信息");
                return;
            }
            //var data = {cypher: _this.cyphertext};
            
             var data = $('#txt').val().trim();
            
             //alert(data)
            var data2 = {cypher:"match (n)-[r]-(s) where n.name='"+data+"' return n,r,s"}
            // alert(data2)
            $.ajax({
                data: data2,
                type: "POST",
                url: "http://localhost:8080/" + "getcypherresult",
        ...
        initEditor() {
            ...
            _this.editor.customConfig.uploadImgServer = "http://localhost:8080/" + "qiniu/upload"; // 上传图片到服务器
        ...
        initNodeContent() {
            ...
            $.ajax({
                data: data,
                type: "POST",
                url: "http://localhost:8080/" + "getnodecontent",
                success: function (result) {
        ...
        initNodeImage() {
            ...
            $.ajax({
                data: data,
                type: "POST",
                url: "http://localhost:8080/" + "getnodeimage",
                success: function (result) {
                    if (result.code == 200) {
        ...
        getNodeDetail(nodeid) {
            ...
            $.ajax({
                data: data,
                type: "POST",
                url: "http://localhost:8080/" + "getnodedetail",
                success: function (result) {
                    if (result.code == 200) {
                        _this.editorcontent = result.data.content;
        ...
        savenodeimage() {
            ...
            $.ajax({
                ...
                type: "POST",
                url: "http://localhost:8080/" + "savenodeimage",
                success: function (result) {
                    if (result.code == 200) {
                        _this.$message({
                            message: '操作成功',
        ...
        savenodecontent() {
            ...
            $.ajax({
                ...
                contentType: 'application/json; charset=UTF-8',
                type: "POST",
                url: "http://localhost:8080/" + "savenodecontent",
                success: function (result) {
                    if (result.code == 200) {
                        _this.$message({
                            message: '操作成功',
        ...
        getdomaingraph() {
            ...
            $.ajax({
                data: data,
                type: "POST",
                url: "http://localhost:8080/" + "getdomaingraph",
                success: function (result) {
                    if (result.code == 200) {
        ...
        getcurrentnodeinfo(node) {
            ...
            $.ajax({
                data: data,
                type: "POST",
                url: "http://localhost:8080/" + "getrelationnodecount",
                success: function (result) {
                    if (result.code == 200) {
        ...
        getmorenode() {
            ...
            $.ajax({
                data: data,
                type: "POST",
                url: "http://localhost:8080/" + "getmorerelationnode",
                success: function (result) {
                    if (result.code == 200) {
        ...
        deletedomain(id,value) {
            ...
            _this.$confirm('此操作将删除该标签及其下节点和关系(不可恢复), 是否继续?', '三思而后行', {
                ...
                $.ajax({
                    data: data,
                    type: "POST",
                    url: "http://localhost:8080/" + "deletedomain",
                    success: function (result) {
                        if (result.code == 200) {
                            _this.getlabels();
                            _this.domain = "";
        ...
        createdomain(value) {
            ...
            _this.$prompt('请输入领域名称', '提示', {
                ...
                $.ajax({
                    data: data,
                    type: "POST",
                    url: "http://localhost:8080/" + "createdomain",
                    success: function (result) {
                        if (result.code == 200) {
                            _this.getlabels();
                            _this.domain = value;
        ...
        getlabels() {
            ...
            $.ajax({
                data: data,
                type: "POST",
                //url: "http://localhost:8080/"+"getlabels",
                url: "http://localhost:8080/" + "getgraph",
                success: function (result) {
                    if (result.code == 200) {
        ...
        showNodeDetail(d){
            ...
            var res = "<p class='nodedetailcss'>name</p><p>"+d['name']+"</p><br><p class='nodedetailcss'>detail</p><p>"+detail+"</p><br>";
            
            for(var a in d){
                if(a=='fixed'||a=='fx'||a=='fy'||a=='index'||a=='r'||a=='vx'||a=='vy'||a=='x'||a=='y'||a=='uuid'||a=='name'||a=='detail'){
                    continue
                }else{
                        d[a] = (typeof(d[a]) == "undefined")?"":d[a];
                        res=res+"<p class='nodedetailcss'>"+a+"</p>"+"<p>"+d[a]+"</p><br>"
                }
            }
            this.$alert(res, {
                title:"知识详情",
        ...
        createnode() {
            ...
            $.ajax({
                ...
                type: "POST",
                traditional: true,
                url: "http://localhost:8080/" + "createnode",
                success: function (result) {
                    if (result.code == 200) {
        ...
        dragended(d) {
            ...
            $.ajax({
                data: ajaxdata,
                type: "POST",
                url: "http://localhost:8080/"+"updateCorrdOfNode",
                success: function (result) {
                    if (result.code == 200) {
                    }
                },
        ...
        drawnode(node) {
            ...
            nodeEnter.attr("fill", function (d) {
                ...
                if(d.label=="hadoop"){
                    return "#FFB6C1";
                }else if(d.label=="command"){
                    return "#00FA9A";
                }else if(d.label=="configuration_file"){
                    return "#BA55D3";
                }else if(d.label=="component"){
                    return "#1E90FF";
                }else if(d.label=="tools"){
                    return "#FFE4C4";
                }else{
                    return "#ff4500";
                }
        ...
        deletelink() {
            ...
            this.$confirm('此操作将删除该关系(不可恢复), 是否继续?', '三思而后行', {
                ...
                $.ajax({
                    data: data,
                    type: "POST",
                    url: "http://localhost:8080/" + "deletelink",
                    success: function (result) {
                        if (result.code == 200) {
                            var j = -1;
                            for (var i = 0; i < _this.graph.links.length; i++) {
        ...
        createlink(sourceId, targetId, ship) {
            ...
            $.ajax({
                data: data,
                type: "POST",
                url: "http://localhost:8080/" + "createlink",
                success: function (result) {
                    if (result.code == 200) {
        ...
        updatelinkName() {
            ...
            _this.$prompt('请输入关系名称', '提示', {
                ...
                $.ajax({
                    data: data,
                    type: "POST",
                    url: "http://localhost:8080/" + "updatelink",
                    success: function (result) {
                        if (result.code == 200) {
                            var newship = result.data;
        ...
        updatenodename(d) {
            ...
            _this.$prompt('编辑节点名称', '提示', {
                ...
                $.ajax({
                    data: data,
                    type: "POST",
                    url: "http://localhost:8080/" + "updatenodename",
                    success: function (result) {
                        if (result.code == 200) {
                            if (d.uuid != 0) {
                                for (var i = 0; i < _this.graph.nodes.length; i++) {
